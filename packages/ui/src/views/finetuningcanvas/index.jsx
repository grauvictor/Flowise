...
// material-ui
import { Toolbar, Box, AppBar, Button, Fab, ButtonBase, Avatar } from '@mui/material'
...
// icons
import { IconX, IconRefreshAlert, IconRun } from '@tabler/icons-react'
...
const Canvas = () => {
    ...
    const chatflowId =
        URLpath[URLpath.length - 1] === 'canvas' || URLpath[URLpath.length - 1] === 'agentcanvas' || URLpath[URLpath.length - 1] === 'finetuningcanvas' ? '' : URLpath[URLpath.length - 1]
    const isAgentCanvas = URLpath.includes('agentcanvas') ? true : false
    const canvasTitle = 'Finetuning'
    ...
    useEffect(() => {
        if (createNewChatflowApi.data) {
            ...
            dispatch({ type: SET_CHATFLOW, chatflow })
            saveChatflowSuccess()
            window.history.replaceState(state, null, `/finetuningcanvas/${chatflow.id}`)
        ...
        <>
            <Box>
                ...
                >
                    ...
                    <ButtonBase title='Run finetuning' sx={{ borderRadius: '50%' }}>
                                    <Avatar
                                        variant='rounded'
                                        sx={{
                                        ...
                                        onClick={() => onClickRun()}
                                    >
                                        <IconRun stroke={1.5} size='1.3rem' />
                                    </Avatar>
                                </ButtonBase>
                </AppBar>
